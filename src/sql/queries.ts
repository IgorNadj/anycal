...
import type { DatabaseSync } from "node:sqlite";
 ...
 export function selectUserUuidByEmail(db: DatabaseSync, email: string): { uuid?: string } | undefined {
   return db
     .prepare("SELECT uuid FROM user WHERE email = ?")
     .get(email) as { uuid?: string } | undefined;
 }

 export function selectUserAuthByEmail(
   db: DatabaseSync,
   email: string,
 ): { uuid: string; email: string; passwordHash: string; passwordSalt: string } | undefined {
   return db
     .prepare("SELECT uuid, email, passwordHash, passwordSalt FROM user WHERE email = ?")
     .get(email) as
     | { uuid: string; email: string; passwordHash: string; passwordSalt: string }
     | undefined;
 }
