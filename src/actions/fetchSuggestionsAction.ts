...
export const fetchSuggestionsAction = async (input: string): Promise<Suggestion[]> => {
  ...
  const config = {
    ...
    systemInstruction: `You are an API endpoint for a client interface which is used for making 
      ...
      to find when that thing is, and return the date. If the thing is ambiguous, return multiple 
      suggestions, each with a title and date. The date should be in ISO8601 RFC3339 format. 
      
      Return only the json, nothing else. Don't return any events 
      before ${format(add(new Date(), { days: 1 }), "yyyy-MM-dd")}. 
      
      The response structure should be: "{ suggestions: [{ title: string, date: string }] }". 
  ...
  // console.log("LLM Response text: ", response.text);
